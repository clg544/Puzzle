...
public class PuzzleAI : MonoBehaviour
{
    ...
    **/
    ...
    /**
     * StateNode BuildGameState()
     * Constructs and returns the game state from the game board for which 
     ...
     *      rightState, belowState, or rotated states.  This leads the tree to the best possible final state.
     *      
    ...
    **/
    ...
    public StateNode Rotate(StateNode inState)
    {
        ...
        switch (shapeDescriptor)
        {
            ...
            case (1):
                // Out of Bounds?
                if (TileRight[0] >= boardWidth || TileAbove[1] >= boardHeight)
                {
                    return null;
                }
            ...
            case (2):
                // Out of Bounds?
                if (TileBelow[1] < boardBottom || TileAbove[1] >= boardHeight)
                    return null;
            ...
            // Below Only
            case (4):
                if (TileLeft[0] < boardLeft || TileAbove[1] >= boardHeight)
                    return null;
            ...
            // Left Only
            case (8):
                if (TileAbove[1] >= boardHeight || TileAbove[1] >= boardHeight)
                    return null;
            ...
            // Up & Right (1 + 2)
            case (3):
                if (TileAbove[1] >= boardHeight || TileRight[0] >= boardWidth || TileAbove[1] >= boardHeight)
                    return null;
            ...
            // Bottom & Right (2 + 4)
            case (6):
                if (TileRight[0] >= boardWidth || TileBelow[1] < boardBottom || TileAbove[1] >= boardHeight)
                    return null;
            ...
            // Bottom & Left (4 + 8)
            case (12):
                if (TileBelow[1] < boardBottom || TileLeft[0] < boardLeft || TileAbove[1] >= boardHeight)
                    return null;
            ...
            // Up & Left (8 + 1)
            case (9):
                if (TileAbove[1] >= boardHeight || TileLeft[0] < boardLeft || TileAbove[1] >= boardHeight)
                    return null;
            ...
            case (10):
                ...
                if (TileAbove[1] >= boardHeight || TileBelow[1] < 0)
                    return null;
            ...
            // Above and Below (1 + 4)
            case (5):
                if (TileAbove[1] >= boardHeight || TileAbove[1] < boardBottom || TileAbove[1] >= boardHeight)
                    return null;

                if (TileRight[0] >= boardWidth || TileLeft[0] < 0)
                    return null;
    ...
    **/
    ...
    #endregion
    #region State_Evaluators


    public int EvaluateColor(StateNode state, TileState color){

        // List of all coordinates to count
        LinkedList<int[]> ColorList = new LinkedList<int[]>();
        ...
        for (int x = 0; x < boardWidth; x++)
        {
            if(state.board[x, 0].state == color)
            {
                ColorList.AddLast(new int[] { x, 0 });
                visited[x, 0] = true;
            }
        }
        ...
        {
            curX = ColorList.First.Value[0];
            curY = ColorList.First.Value[1];
            colorCount++;

            // Add unvisited neighbors to the list
            if (curX > 0)
            {
                if (state.board[curX - 1, curY].state == color &&
                        !visited[curX - 1, curY])
                {
                    ColorList.AddLast(new int[] { curX - 1, curY });
                    visited[curX - 1, curY] = true;
                }
            }

            if (curX < boardWidth - 1)
            {
                if (state.board[curX + 1, curY].state == color &&
                        !visited[curX + 1, curY])
                {
            ...
            if(curY > 0){
                if (state.board[curX, curY - 1].state == color &&
                        !visited[curX, curY - 1])
                {
                    ColorList.AddLast(new int[] { curX, curY - 1 });
                    visited[curX, curY - 1] = true;
                }
            }

            if(curY < boardHeight - 1){
                if (state.board[curX, curY + 1].state == color &&
                        !visited[curX, curY + 1])
                {
                    ColorList.AddLast(new int[] { curX, curY + 1 });
        ...
        LinkedList<BoolSquare> squareList = myGameBoard.ApproximateRectangles(visited);
        colorCount += myGameBoard.ScoreBoolSquareList(squareList);

        return colorCount;
    }

    /**
     *  int EvaluateAllColors(StateNode state)
     *
     *  Find the value of all color clusters that are touching the board floor.
     */
    ...
    public int[,] GenerateColorPotentialGrid(StateNode inState, TileState color)
    {
        if (color == TileState.EMPTY)
            return null;

        const int BEST_SCORE = 100;
        const int NUM_ADJACENT = 8;

        SubState[,] curState = inState.board;
        int[,] Potential = new int[boardWidth, boardHeight];
        
        int x, y;
        bool hasLeft, hasUp, hasRight, hasDown;
        int adjacent;
        for (int i = 0; i < boardWidth * boardHeight; i++)
        {
            x = i % boardWidth;
            y = i / boardHeight;
            adjacent = 0;
            if (curState[x, y].state == TileState.EMPTY)
            {
                if (x <= 0)
                    hasLeft = false;
                else
                ...
                if (hasUp)
                {
                    if (curState[x, y + 1].state == color)
                        adjacent++;

                    if (hasLeft)
                    {
                        if (curState[x - 1, y + 1].state == color)
                            adjacent++;
                    }
            ...
            Potential[x, y] = adjacent * (BEST_SCORE / NUM_ADJACENT);
        }
    ...
    /**
     * int QuantifyBoardState(StateNode gameState)
     *
     * For the Given stateNode, evaluate the relative value based on the state
     *     of the board.
     */
    public void QuantifyBoardState(StateNode gameState)
    {
        int score = 0;
        int numEmpty = 0;
        ...
        gameState.numEmpty = numEmpty;

        gameState.redValue = EvaluateColor(gameState, TileState.RED);
        gameState.greenValue = EvaluateColor(gameState, TileState.GREEN);
        gameState.blueValue = EvaluateColor(gameState, TileState.BLUE);

        score += gameState.redValue;
        score += gameState.blueValue;
        score += gameState.greenValue;
    ...
    public void TestAll()
    {
        ...
        int emptyValues = 0;
        StateNode testNode = new StateNode(testBoard);
        QuantifyBoardState(testNode);
        ...
        ************/
        ...
        testNode = new StateNode(testBoard);
        QuantifyBoardState(new StateNode(testBoard));
    ...
    public void TestColorPotentialGrid()
    {
        if (!Debug.isDebugBuild)
            return;

        SubState[,] testBoard = new SubState[boardWidth, boardHeight];
        ...
        for(int i = 90; i >= 0; i--)
        {
            ...
            if(testBoard[x, y].state == TileState.EMPTY)
            {
                str += "E";
            }
            else
        ...
        int[,] testPotential = GenerateColorPotentialGrid(testState, TileState.RED);
        str = "";
        for (int i = 90; i >= 0; i--)
        {
            x = i % boardWidth;
    ...
    void Update()
    {
        ...
        switch (ChooseMove())
        {
            ...
            case MoveChoice.FALL:
                if (myCharacter.dropTime > myCharacter.fastTileDropRate)
